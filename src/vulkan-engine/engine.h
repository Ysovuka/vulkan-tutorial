#pragma once
#ifndef VulkanEngine_Class
#define VulkanEngine_Class
...
#include "debugReportCallbackCreateInfo.h"
...
public:
	VulkanEngine() {}
	VulkanEngine(char* applicationName, uint32_t applicationVersion)
	{
		APPLICATION_NAME = applicationName;
		APPLICATION_VERSION = applicationVersion;
	}
	VulkanEngine(char* applicationName, char* engineName,
...
private:
	...
	char* APPLICATION_NAME = "Vulkan Tutorial";
	...
	DebugReportCallbackCreateInfo* debugInfo;
	...
	void cleanup()
	{
		...
		if (debugInfo != nullptr)
			debugInfo->DetachDebugReport(instance);
	...
	void createInstance()
	{
		...
		ApplicationInfo applicationInfo(APPLICATION_NAME, ENGINE_NAME,
		...
		VkResult results = vkCreateInstance(&instanceInfo, nullptr, &instance);
	...
	void setupDebugCallback()
	{
		...
		debugInfo = new DebugReportCallbackCreateInfo(debugCallback);

		if ((debugInfo->AttachDebugReport(instance)) != VK_SUCCESS)
		{
	...
	int rateDeviceSuitability(VkPhysicalDevice device)
	{
		...
		if (!checkDeviceExtensionSupport(device))
		{
			...
			if (swapChainSupport.presentationModes.empty())
			{
				return 0;
			}

			score += swapChainSupport.formats.size();
			score += swapChainSupport.presentationModes.size();
		}
	...
	std::vector<const char*> getRequiredExtensions()
	{
		...
		if (enabledValidationLayers) {
			std::cout << "Extensions: " << VK_EXT_DEBUG_REPORT_EXTENSION_NAME << std::endl;
	...
	void drawFrame()
	{
		...
		uint32_t imageIndex;
		VkResult aquireNextImageResult = vkAcquireNextImageKHR(device, swapChain, std::numeric_limits<uint64_t>::max(), imageAvailableSemaphore, VK_NULL_HANDLE, &imageIndex);

		if (aquireNextImageResult == VK_ERROR_OUT_OF_DATE_KHR)
		{
			recreateSwapChain();
			return;
		}
		else if (aquireNextImageResult != VK_SUCCESS && aquireNextImageResult != VK_SUBOPTIMAL_KHR)
		{
		...
		VkResult presentationResult = vkQueuePresentKHR(presentationQueue, &presentInfo);
